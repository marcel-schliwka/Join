...
function htmlTemplateToDo(element, i, priority) {
  return `<div onclick="boardOpenPopUpTask(${i})" draggable="true" ondragstart="startDragging(${i})" class="bgWhite2 cursorPointer boxShadow border rounded-5 p-2 my-3 d-flex flex-column align-items-start">
...
function htmlTemplateDone(element, i, priority) {
  return `<div onclick="boardOpenPopUpTask(${i})" draggable="true" ondragstart="startDragging(${i})" class="cursorPointer bgWhite2 boxShadow border rounded-5 p-2 my-3 d-flex flex-column align-items-start">
        <div class="textWhite border rounded-3 px-3 m-2" style="background-color:grey">
            ${element["category"]}
        </div>
        <div class="mt-2 mx-2 bold">
...
function moveTo(status) {
  userObj.tasks[currentDraggedElement]["status"] = status; // z.B. task mit id 1: Das Feld 'status' ändert sich von 'todo' zu 'in progress'
  updateHTML();
}
